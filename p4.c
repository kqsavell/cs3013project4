...
int replace_page(int pid, int v_page)
{
    ...
    on_disk[r_pid][r_vpage] = new_line; // Update page that was swapped to disk
...
int swap(int page, int lineNum)
{
    ...
    if (ptable_flag == 1)
    {
        printf("Put page table for PID %d into swap slot %d", ptable_flag, putLine);
    }
    return putLine;
}
